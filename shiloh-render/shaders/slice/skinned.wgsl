// Skinned PBR vertex path — 4 influences, joint palette

struct FrameUniform {
    view_proj: mat4x4<f32>,
    light_view_proj: mat4x4<f32>,
    camera_pos: vec4<f32>,
    sun_dir: vec4<f32>,
    sun_color: vec4<f32>,
    ambient: vec4<f32>,
    fog: vec4<f32>,
    point0_pos_range: vec4<f32>,
    point0_color: vec4<f32>,
    point1_pos_range: vec4<f32>,
    point1_color: vec4<f32>,
    params: vec4<f32>,
}

struct SkinUniform {
    joints: array<mat4x4<f32>, 64>,
}

@group(0) @binding(0) var<uniform> frame: FrameUniform;
@group(0) @binding(1) var shadow_tex: texture_depth_2d;
@group(0) @binding(2) var shadow_samp: sampler_comparison;
@group(0) @binding(3) var albedo_tex: texture_2d<f32>;
@group(0) @binding(4) var albedo_samp: sampler;
@group(1) @binding(0) var<uniform> skin: SkinUniform;

struct VsIn {
    @location(0) position: vec3<f32>,
    @location(1) normal: vec3<f32>,
    @location(2) uv: vec2<f32>,
    @location(3) color: vec3<f32>,
    @location(4) joints: vec4<u32>,
    @location(5) weights: vec4<f32>,
}

struct VsOut {
    @builtin(position) clip: vec4<f32>,
    @location(0) world_pos: vec3<f32>,
    @location(1) world_n: vec3<f32>,
    @location(2) uv: vec2<f32>,
    @location(3) color: vec3<f32>,
    @location(4) shadow_coord: vec4<f32>,
}

@vertex
fn vs_main(in: VsIn) -> VsOut {
    var skin_m = mat4x4<f32>();
    skin_m = skin.joints[in.joints.x] * in.weights.x;
    skin_m += skin.joints[in.joints.y] * in.weights.y;
    skin_m += skin.joints[in.joints.z] * in.weights.z;
    skin_m += skin.joints[in.joints.w] * in.weights.w;
    let world = skin_m * vec4<f32>(in.position, 1.0);
    var out: VsOut;
    out.clip = frame.view_proj * world;
    out.world_pos = world.xyz;
    out.world_n = normalize((skin_m * vec4<f32>(in.normal, 0.0)).xyz);
    out.uv = in.uv;
    out.color = in.color;
    out.shadow_coord = frame.light_view_proj * world;
    return out;
}

fn shadow_factor(shadow_coord: vec4<f32>) -> f32 {
    var c = shadow_coord;
    c = c / c.w;
    let uv = c.xy * vec2<f32>(0.5, -0.5) + vec2<f32>(0.5, 0.5);
    if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0 || c.z > 1.0) {
        return 1.0;
    }
    return textureSampleCompareLevel(shadow_tex, shadow_samp, uv, c.z - 0.002);
}

@fragment
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    let n = normalize(in.world_n);
    let albedo = textureSample(albedo_tex, albedo_samp, in.uv).rgb * in.color;
    let l = normalize(-frame.sun_dir.xyz);
    let ndotl = max(dot(n, l), 0.0);
    let sh = shadow_factor(in.shadow_coord);
    var rgb = frame.ambient.xyz * albedo + frame.sun_color.xyz * albedo * ndotl * sh;
    let dist = distance(frame.camera_pos.xyz, in.world_pos);
    let fog_f = 1.0 - exp(-frame.fog.w * dist);
    rgb = mix(rgb, frame.fog.xyz, saturate(fog_f));
    return vec4<f32>(rgb, 1.0);
}
